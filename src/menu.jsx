    ...
    <div className="bootstrap-wf">
      <nav className="navbar navbar-expand-md navbar-light bg-light hide">
        <button
          ...
          data-target="#wfNavbar"
          aria-controls="wfNavbar"
          aria-expanded="false"
  ...
  if (menuItem.children.length === 0) {
    return (
      <li className="nav-item">
        <a
          className={["nav-link", menuItem.active ? "active" : ""].join(" ")}
          href={menuItem.link}
        >
          {menuItem.name}
        </a>
...
const menuNavItemDropdown = dropdownMenuItem => {
  return (
    <li className="nav-item dropdown">
      ...
      <div
        className="dropdown-menu dropdown-primary dropright"
        aria-labelledby="navbarDropdownMenuLink"
      >
        {dropdownMenuItem.children.length < COLUMN_DIVIDE_AT && (
          <>
            {dropdownMenuItem.children.map(dropdownMenuItemChild =>
  ...
  return (
    <div className="dropdown-submenu">
      <a
        ...
        href={dropdownMenuItemChild.link}
      >
        {dropdownMenuItemChild.name}
      </a>
      <div className="dropdown-menu">
        {dropdownMenuItemChild.children.map(dropdownMenuItemSubChild =>
